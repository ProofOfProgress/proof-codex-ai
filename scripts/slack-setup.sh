...
echo "=============================================="
echo ""
echo "Three integrations (pick what works):"
echo "  1. @cursor app        — you message Slack → Cloud Agent runs → replies in thread"
echo "  2. Gmail → channel email (Option A) — alerts without bot token / /invite"
echo "  3. Incoming webhook   — bot posts pipeline alerts (alternative)"
echo ""
...
echo ""
echo "--- Part 3: Email into Slack (Option A — no bot token) ---"
echo "  1. #peripheral-ops → Integrations → Email → copy address"
echo "  2. Google → App Password for ops Gmail"
echo "  3. Cursor Secrets → SLACK_CHANNEL_EMAIL, GMAIL_SMTP_USER, GMAIL_SMTP_APP_PASSWORD"
echo "  4. Optional: SLACK_POST_MODE=email"
echo "  5. bash scripts/install.sh && python3 -m shorts_bot.integrations test"
echo "  Guide: docs/FOR_OWNER_SLACK_EMAIL.md"
echo ""
echo "--- Part 4: Webhook (pipeline alerts — alternative) ---"
