...
def test_primary_lane_is_village_eye_worship():
    assert HORROR_LANE_PRIMARY == "village_cult_horror"
    assert "Eye worship" in HORROR_LANE_LABEL
...
def test_lane_compact_rejects_creature_and_animal_cruelty():
    ...
    assert "security cameras" in text.lower()
    assert "fog" in text.lower()
...
def test_analog_color_rules_in_visual_dna():
    ...
    assert HORROR_LANE_LABEL.split()[0] in visual_dna()


def test_world_rules_include_lane():
    assert "Eye worship" in world_rules_compact()
