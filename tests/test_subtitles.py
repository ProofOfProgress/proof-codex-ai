...
def test_captions_use_approved_script_not_asr():
    script = (
        "Your security camera flagged motion at 3:12 AM — you live alone. "
        "You opened the app. The hallway was empty."
    )
    segments = [
        {"start_seconds": 0, "end_seconds": 5, "spoken_text": "Your security camera flag motion"},
        {"start_seconds": 5, "end_seconds": 10, "spoken_text": "You live alone. You open the app."},
        {"start_seconds": 10, "end_seconds": 15, "spoken_text": "The hallway was empty."},
    ]
    srt, _ = build_subtitles_from_manifest(segments, audio_duration=15.0, script=script)
    assert "flagged" in srt
    assert "flag motion" not in srt
