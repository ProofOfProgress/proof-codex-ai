...
def credentials_status_message() -> str:
    ...
    if credentials_configured():
        settings_has_keys = _secret_real(settings.google_client_id) and _secret_real(
            settings.google_client_secret
        )
        cid_t, sec_t = _token_file_oauth_fields()
        if not settings_has_keys and _secret_real(cid_t) and _secret_real(sec_t):
            return f"Google OAuth keys OK (from {token_path().name})"
