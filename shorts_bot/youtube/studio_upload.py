...
def _resolve_upload_fields(
    ...
    *,
    ...
    return draft_id, use_title, use_desc, use_vis


def _preflight_studio_upload(
    *,
    draft_id: int,
    pack_dir: Path | None,
    use_title: str,
    ...
    skip_preflight: bool,
) -> StudioUploadResult | None:
    if skip_preflight or not draft_id or not pack_dir:
        return None
...
def _perform_studio_upload_on_page(
    page,
    video_path: Path,
    *,
    use_title: str,
    ...
    """Run Studio upload steps on an already-authenticated page."""
    ...
    try:
        ...
        try:
            if page.get_by_text(re.compile(r"not made for kids", re.I)).count():
...
def upload_via_studio(
    ...
    *,
    ...
    draft_id, use_title, use_desc, use_vis = _resolve_upload_fields(
        video_path, title=title, description=description, visibility=visibility, pack_dir=pack_dir
    )
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            return _perform_studio_upload_on_page(
                page,
                video_path,
                use_title=use_title,
                use_desc=use_desc,
