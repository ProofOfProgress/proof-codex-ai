"""DEPRECATED — hub Kalodata via desktop helper coordinates + Gemini vision."""
...
def close_duplicate_tabs(max_close: int = 6) -> None:
    """DEPRECATED — ctrl+w can close entire Edge. Use Playwright navigate instead."""
    logger.warning("close_duplicate_tabs is deprecated — use Playwright ensure_product_list")
    ...
    *,
    ...
    for attempt in range(max_attempts):
        ...
        if last_verify.ok:
            ...
            return SessionResult(
                ok=True,
                message=f"Filters applied (legacy desktop): {method}",
                verify=last_verify,
                filter_url=url,
            )
    ...
    return SessionResult(
        ok=False,
        message=f"ABORTED — filters NOT submitted. Issues: {issues}",
        verify=last_verify,
    )
