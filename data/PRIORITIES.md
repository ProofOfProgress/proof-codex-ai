...
| **3** | **Better visuals & sync** | Video must *look* right and match the voice beat-by-beat | **Full-motion I2V** (FLUX still → MiniMax clip per beat, `VISUAL_STYLE=ai_video`), Gemini transcript timing, ffmpeg captions, **vision QC** | **Switching default** — slideshow → motion clips; stick figure fallback if no Replicate key |
...
3. **Visual sync** — motion clips match the voice beat-by-beat (not a static slideshow)  
