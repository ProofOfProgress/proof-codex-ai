...
| `GMAIL_SMTP_USER` | Runtime Secret | Ops Gmail (AlphaBeta / Slack only) | `[REDACTED]` | **Do not use for B2B** |
| `GMAIL_SMTP_APP_PASSWORD` | Runtime Secret | Google App Password (ops) | `16-char password` | Slack alerts only |
| `B2B_SMTP_USER` | Runtime Secret | Dedicated outreach inbox | `outreach@rapidtoolreview.com` | `docs/FOR_OWNER_B2B_EMAIL.md` |
| `B2B_SMTP_APP_PASSWORD` | Runtime Secret | App password (outreach inbox) | `16-char password` | Separate from ops Gmail |
| `B2B_TEST_EMAIL` | Environment Variable | Owner inbox for test pings | `you@example.com` | Test lands here, not outreach inbox |
| `B2B_EMAIL_ENABLED` | Environment Variable | `true` / `false` | `false` | Enable real B2B sends |
| `B2B_EMAIL_FROM_NAME` | Environment Variable | Display name on From: | `Kim` | e.g. Kim &lt;outreach@...&gt; |
